...
export const getUserEmail = async (userId: number): Promise<string> => {
    try {
        ...
        console.error(`Failed to fetch email for userId ${userId}:`, error.message);
...
router.post("/project/created", async (req, res) => {
    ...
    try {
        ...
        const error = err as Error; 
        console.error("Error processing project creation notification:", error.message);
...
const API_GATEWAY_URL = 'https://xyz123.execute-api.us-east-2.amazonaws.com/send-email'; 

export const sendEmail = async (toEmail: string, subject: string, message: string): Promise<void> => {
    try {
